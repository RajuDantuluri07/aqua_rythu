-- Smart Feed Initialization: safe onboarding for active ponds past blind phase
--
-- Business rules:
--   nursery seed: DOC > 10  → skip blind rounds, farmer provides current data
--   hatchery seed: DOC > 30 → skip blind rounds, farmer provides current data
--
-- CRITICAL: DO NOT generate fake history (DOC 1 → current DOC feed rows).
-- When p_skip_feed_rounds is true the pond is created with zero feed_rounds.
-- The Dart smart-feed engine generates today's rounds on first dashboard load.
--
-- Changes:
--   1. Add smart init tracking columns to ponds
--   2. Replace create_pond_with_feed_plan RPC with version supporting
--      p_skip_feed_rounds (when true: pond created, feed round loop skipped)

-- ── 1. Add columns to ponds ───────────────────────────────────────────────
ALTER TABLE public.ponds
  ADD COLUMN IF NOT EXISTS smart_feed_initialized BOOLEAN DEFAULT false,
  ADD COLUMN IF NOT EXISTS initialization_doc INTEGER,
  ADD COLUMN IF NOT EXISTS survival_pct DOUBLE PRECISION;

-- ── 2. Drop all existing overloads ───────────────────────────────────────
DO $$
DECLARE
  v_sig TEXT;
BEGIN
...
-- ── 3. Recreate RPC with p_skip_feed_rounds support ──────────────────────
CREATE OR REPLACE FUNCTION public.create_pond_with_feed_plan(
  ...
  p_skip_feed_rounds BOOLEAN DEFAULT FALSE
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- ── Step 2: Skip feed rounds for smart-init ponds ───────────────────────
  -- No historical rows are created. The smart feed engine generates today's
  -- rounds on first dashboard load after the farmer completes initialization.
  IF p_skip_feed_rounds THEN
    RETURN jsonb_build_object(
      'success',             TRUE,
      'pond_id',             v_pond_id,
      'feed_rounds_created', 0,
      'smart_init',          TRUE
    );
  END IF;

  -- ── Step 3: Generate blind-phase feed_rounds ────────────────────────────
  ...
  FOR v_doc IN 1 .. v_end_doc LOOP
    ...
    -- Base feed rate (kg / 100k seeds) — mirrors FeedBaseRate._legacyRateFor
...
GRANT EXECUTE ON FUNCTION public.create_pond_with_feed_plan(
  UUID, TEXT, DOUBLE PRECISION, DATE, INTEGER, INTEGER, INTEGER, TEXT, UUID, UUID, BOOLEAN
) TO authenticated;

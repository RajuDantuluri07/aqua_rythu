-- P0 FIX: Align create_pond_with_feed_plan RPC with Flutter client signature.
--
-- Production DB had the old 8-param signature (p_user_id, returns TEXT).
-- Flutter client sends 10 params and expects JSONB back.
-- PostgREST could not match signature → PGRST202 → pond creation broken for all users.
--
-- Fix:
--   1. Add operation_id column to ponds (idempotency key).
--   2. Drop all old function overloads.
--   3. Create correct atomic RPC matching Flutter contract.

-- ── 1. Add operation_id to ponds ─────────────────────────────────────────────
...
BEGIN
  ...
  IF p_seed_count IS NULL OR p_seed_count < 1000 THEN
    RETURN jsonb_build_object('success', FALSE, 'error', 'Seed count must be at least 1000');
  END IF;
  ...
  -- Idempotency: same operation_id → return existing pond without duplicate insert
  ...
  -- Blind-phase feed_rounds: nursery DOC 1–10, hatchery DOC 1–30
  ...
  FOR v_doc IN 1 .. v_end_doc LOOP
    ...
    IF p_stocking_type = 'nursery' THEN
      ...
      END;
    ELSE
      -- hatchery cumulative-increment formula; tier anchors: DOC7=2.7, DOC14=4.8, DOC21=7.6

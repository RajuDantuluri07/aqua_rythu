...
Future<void> testDifferenceCalculation() async {
  ...
  print(
      '✅ Formula implemented: ((feed_entered - calculated_feed) / calculated_feed) * 100');
  print('✅ Location: _buildFeedComparisonSection() in feed_debug_panel.dart');
  print(
      '✅ Edge case handled: calculated_feed > 0 check prevents division by zero');
  ...
  for (final case_ in testCases) {
    ...
    final actual =
        calculated > 0 ? ((entered - calculated) / calculated * 100) : 0.0;

    final status = (actual - expected).abs() < 0.1 ? '✅' : '❌';
    print(
        '   $status Entered: ${entered}kg, Calc: ${calculated}kg → ${actual.toStringAsFixed(1)}% (expected: ${expected.toStringAsFixed(1)}%)');
  }
...
Future<void> testDataSources() async {
  ...
  print(
      '   Verification: loadTodayFeed() refreshes state from DB after transaction');
...
Future<void> testUILabels() async {
  ...
  print(
      '   "FEED COMPARISON - VERIFIED DATA SOURCES" - Emphasizes reliability');
...
Future<void> testColorCoding() async {
  ...
  for (final test in colorTests) {
    ...
    print(
        '   $status ${diff.toStringAsFixed(1)}% → $actual (expected: $expected)');
  }
...
double calculateDifference(double entered, double calculated) {
  return calculated > 0 ? ((entered - calculated) / calculated * 100) : 0.0;
}

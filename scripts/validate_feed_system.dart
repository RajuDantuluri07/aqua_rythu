...
/// Test A: Double Tap Prevention
Future<void> testDoubleTapPrevention(SupabaseClient supabase, String pondId, int doc) async {
  print('\n🔴 TEST A: Double Tap Prevention');
  print('Testing rapid double tap simulation...');
  
  // Clear any existing data
  await supabase.from('feed_logs').delete().eq('pond_id', pondId).eq('doc', doc);
  await supabase.from('feed_rounds').delete().eq('pond_id', pondId).eq('doc', doc);
  ...
  if (feedLogs.length == 1 && feedRounds.length == 1) {
    ...
    throw Exception('Double tap prevention failed - found ${feedLogs.length} logs and ${feedRounds.length} rounds');
  }
}

/// Test B: Atomic Save Behavior
Future<void> testAtomicSaveBehavior(SupabaseClient supabase, String pondId, int doc) async {
  print('\n🔴 TEST B: Atomic Save Behavior');
  ...
  if ((feedLogs.length == 0 && feedRounds.length == 0) || 
    ...
    throw Exception('Atomic save behavior failed - inconsistent state detected');
  }
}

/// Test C: Sequential Feed Calculations
Future<void> testSequentialFeedCalculations(SupabaseClient supabase, String pondId, int doc) async {
  print('\n🔴 TEST C: Sequential Feed Calculations');
...
/// Test D: Network Failure Handling
Future<void> testNetworkFailureHandling(SupabaseClient supabase, String pondId, int doc) async {
  print('\n🔴 TEST D: Network Failure Handling');
  ...
  try {
    await simulateFeedOperation(supabase, pondId, doc, 4, -5.0); // Invalid amount
  } catch (e) {
...
/// Final System Check
Future<void> finalSystemCheck(SupabaseClient supabase, String pondId, int doc) async {
...
Future<void> simulateFeedOperation(SupabaseClient supabase, String pondId, int doc, int round, double amount) async {
  if (amount <= 0) {
